					...
					if(plugin()->is_pro_preview())
						{
							...
							echo '      <p>'.__('Quick Cache stores its cache files for each page of your site, based on the PROTOCOL://HOST/URI that it\'s caching. If you want to cache multiple versions of each page you can add a Salt to the mix. So, instead of just <code>$_SERVER[\'HTTPS\'].$_SERVER[\'HTTP_HOST\'].$_SERVER[\'REQUEST_URI\']</code>, you might prefer something like <code>$_SERVER[\'HTTPS\'].$_SERVER[\'HTTP_HOST\'].$_SERVER[\'REQUEST_URI\'].(string)@$_COOKIE[\'my_cookie\']</code>; where the Version Salt is <code>(string)@$_COOKIE[\'my_cookie\']</code>.', plugin()->text_domain).'</p>'."\n";
							...
							echo '      <table style="width:100%;"><tr><td style="width:1px; font-weight:bold; white-space:nowrap;">$_SERVER[\'HTTPS\'].$_SERVER[\'HTTP_HOST\'].$_SERVER[\'REQUEST_URI\'].</td><td><input type="text" name="'.esc_attr(__NAMESPACE__).'[save_options][version_salt]" value="" class="monospace" /></td><td style="width:1px; font-weight:bold; white-space:nowrap;"></td></tr></table>'."\n";
