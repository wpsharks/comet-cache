	...
	class menu_pages // Plugin options.
	{
		public function options()
		{
			...
			if(plugin()->is_pro_preview())
			{
				...
				echo '      <p><select name="'.esc_attr(__NAMESPACE__).'[save_options][cache_purge_term_post_tag_enable]">'."\n";
				...
				echo '            <option value="1">'.__('Yes, if any single Post/Page is purged/reset; also purge any associated custom Term archive views.', plugin()->text_domain).'</option>'."\n";
				echo '            <option value="0" selected="selected">'.__('No, my site doesn\'t use any custom Terms and/or I don\'t have any custom Term archive views.', plugin()->text_domain).'</option>'."\n";
				echo '         </select></p>'."\n";
				echo '   </div>'."\n";
			...
			if(plugin()->is_pro_preview())
			{
				...
				echo '               <option value="1" selected="selected">'.__('Yes, combine CSS from &lt;head&gt; and &lt;body&gt; into fewer files.', plugin()->text_domain).'</option>'."\n";
				...
				echo '               <option value="1" selected="selected">'.__('Yes, compress the code in any unified CSS files.', plugin()->text_domain).'</option>'."\n";
				...
				echo '               <option value="1" selected="selected">'.__('Yes, combine JS from &lt;head&gt; into fewer files.', plugin()->text_domain).'</option>'."\n";
				...
				echo '               <option value="1" selected="selected">'.__('Yes, combine JS footer scripts into fewer files.', plugin()->text_domain).'</option>'."\n";
				...
				echo '               <option value="1" selected="selected">'.__('Yes, combine CSS/JS from remote resources too.', plugin()->text_domain).'</option>'."\n";
				...
				echo '               <option value="1" selected="selected">'.__('Yes, compress the code in any unified JS files.', plugin()->text_domain).'</option>'."\n";
				...
				echo '               <option value="1" selected="selected">'.__('Yes, compress inline JavaScript snippets.', plugin()->text_domain).'</option>'."\n";
				...
				echo '               <option value="1" selected="selected">'.__('Yes, compress (remove extra whitespace) in the final HTML code too.', plugin()->text_domain).'</option>'."\n";
				...
				echo '         <p class="info" style="display:block;">'.__('<strong>Note:</strong> please remember that your entries here should be formatted as a line-delimited list; e.g. one exclusion pattern per line.', plugin()->text_domain).'</p>'."\n";
				echo '         <hr />'."\n";
				echo '         <h3>'.__('HTML Compression Cache Expiration', plugin()->text_domain).'</h3>'."\n";
			...
			if(plugin()->is_pro_preview())
			{
				...
				echo '   <div class="plugin-menu-page-panel-heading">'."\n";
				echo '      <i class="fa fa-gears"></i> '.__('Dynamic Version Salt', plugin()->text_domain)."\n";
				echo '   </div>'."\n";
				...
				echo '      <p>'.__('<em>Note: Understanding the Quick Cache <a href="http://www.websharks-inc.com/r/quick-cache-branched-cache-structure-wiki/" target="_blank">Branched Cache Structure</a> is a prerequisite to understanding how Dynamic Version Salts are added to the mix.</em>', plugin()->text_domain).'</p>'."\n";
				echo '      <p>'.__('A Version Salt gives you the ability to dynamically create multiple variations of the cache, and those dynamic variations will be served on subsequent visits; e.g. if a visitor has a specific cookie (of a certain value) they will see pages which were cached with that version (i.e. w/ that Version Salt: the value of the cookie). A Version Salt can really be anything.', plugin()->text_domain).'</p>'."\n";
				echo '      <p>'.__('A Version Salt can be a single variable like <code>$_COOKIE[\'my_cookie\']</code>, or it can be a combination of multiple variables, like <code>$_COOKIE[\'my_cookie\'].$_COOKIE[\'my_other_cookie\']</code>. (When using multiple variables, please separate them with a dot, as shown in the example.)', plugin()->text_domain).'</p>'."\n";
				echo '      <p>'.__('Experts could even use PHP ternary expressions that evaluate into something. For example: <code>((preg_match(\'/iPhone/i\', $_SERVER[\'HTTP_USER_AGENT\'])) ? \'iPhones\' : \'\')</code>. This would force a separate version of the cache to be created for iPhones (e.g., <code>/cache/PROTOCOL/HOST/REQUEST-URI.v/iPhones.html</code>).', plugin()->text_domain).'</p>'."\n";
				echo '      <p>'.__('For more documentation, please see <a href="http://www.websharks-inc.com/r/quick-cache-dynamic-version-salts-wiki/" target="_blank">Dynamic Version Salts</a>.', plugin()->text_domain).'</p>'."\n";
				echo '      <hr />'."\n";
				echo '      <h3>'.__('Create a Dynamic Version Salt For Quick Cache? &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span style="font-size:90%; opacity:0.5;">150% OPTIONAL</span>', plugin()->text_domain).'</h3>'."\n";
				...
				echo '      <p class="info" style="display:block;">'.__('If you\'ve enabled a separate cache for each user (optional) that\'s perfectly OK. A Version Salt works with user caching too.', plugin()->text_domain).'</p>'."\n";
				echo '   </div>'."\n";
