					...
					if(!empty($_REQUEST[__NAMESPACE__.'_pro_preview']))
						{
							...
							echo '   <i class="fa fa-eye"></i> '.__('<strong>Pro Version Preview (below)</strong> ~ New option panels. Please explore before <a href="http://www.websharks-inc.com/r/quick-cache-pro/" target="_blank">upgrading <i class="fa fa-heart-o"></i></a>.<br />NOTE: the free version of Quick Cache (this LITE version); is more-than-adequate for most sites. Please upgrade only if you desire the advanced features or would like to support the developer of this plugin.', plugin()->text_domain)."\n";
							echo '</div>'."\n";
						}
					...
					if(plugin()->is_pro_preview())
						{
							...
							echo '   <div class="plugin-menu-page-panel-body clearfix">'."\n";
							echo '      <h3>'.__('Clearing the Cache (Manually and Automatically)', plugin()->text_domain).'</h3>'."\n";
							...
							echo '      <p>'.__('Depending on the structure of your site, there could be many reasons to clear the cache. However, the most common reasons are related to Post/Page edits or deletions, Category/Tag edits or deletions, and Theme changes. If you edit a Post/Page (or delete one), Quick Cache will automatically purge the cache file(s) associated with that content, so that a new updated version of the cache will be created automatically the next time this content is accessed. Simple updates like this occur each time you make changes in the Dashboard, and Quick Cache will notify you of these as they occur. Quick Cache monitors changes to Posts (of any kind, including Pages), Categories, Tags, Links, Themes (even Users); and more. Notifications in the Dashboard regarding these detections can be enabled/disabled.', plugin()->text_domain).'</p>'."\n";
							echo '      <p><select name="'.esc_attr(__NAMESPACE__).'[save_options][admin_bar_enable]">'."\n";
							echo '            <option value="1" selected="selected">'.__('Yes, enable &quot;Clear Cache&quot; button in the WordPress admin bar.', plugin()->text_domain).'</option>'."\n";
							...
							echo '            <option value="1" selected="selected">'.__('Yes, enable Quick Cache notifications in the Dashboard when changes are detected &amp; the cache is cleared.', plugin()->text_domain).'</option>'."\n";
